...
function tackleAdvisory(resolution, advisory, params) {
  const { packageJSON, yarnVersion } = params;
  ...
  for (const currPackage of chain) {
    const packageRef = `${currPackage}@${dependencies[currPackage]}`;
    const lockEntry = result.object[packageRef];
    if (!lockEntry) {
      console.error(`Failed to trace dependency chain in lockfile for ${currPackage}`);
      process.exit(1);
    }
  ...
  console.log(
    '    ',
    chain
      .map((currPackage) => chalk.yellow(currPackage))
      .map((currPackage, index) =>
        index === chain.length - 1 ? chalk.bold(currPackage) : currPackage,
      )
      .join(chalk.grey(' --> ')),
  );
  console.log('LOCKENTIRES: ', lockEntryChain);
  // For each entry:
  ...
  lockEntryTries: for (const [entryIndex, entry] of lockEntryChain.entries()) {
    ...
    for (const [packageIndex] of chainToResolve.entries()) {
      ...
      const parentPackage = chainToResolve[packageIndex + 1];
      if (parentPackage) {
        if (deps[parentPackage]) {
          packageRef = `${parentPackage}@${deps[parentPackage]}`;
        } else {
          packageRef = null;
          break;
        }
